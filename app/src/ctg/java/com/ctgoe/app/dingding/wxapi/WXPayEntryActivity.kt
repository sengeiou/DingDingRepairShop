...
class WXPayEntryActivity : BaseActivity(), IWXAPIEventHandler {
    ...
    override fun onResp(resp: BaseResp?) {
        MyLog.d("CTGCTG")
        MyLog.d("微信支付回调:${resp?.errStr?.handlerNull()}")
        MyLog.d("微信支付回调:${resp?.errCode?.handlerNull()}")
        MyLog.d("微信支付回调:${getGson().toJson(resp)}")
        if (resp?.type == ConstantsAPI.COMMAND_PAY_BY_WX) {
            ...
            when (resp.errCode) {
                0 -> {
                    ...
                    //储值卡充值
                    else if(BaseApplication.currentPayFrom==PayFromEnum.TOP_UP_RECHARGE){
                        var intent = Intent()
                        intent.action = MyConstants.WXPAY_RESULT
