    ...
    public void onDraw(Canvas canvas) {
        ...
        if (keys != null && keys.size() > 0) {
            ...
            for (Keyboard.Key key : keys) {
                if (key.codes[0] == -4) {
                    Drawable dr = getContext().getResources().getDrawable(R.drawable.bg_btn_common);
...
//                        paint.setShadowLayer(0, 0, 0, 0);
                    ...
                    Drawable dr = getContext().getResources().getDrawable(R.drawable.icon_delete_epc);
                    dr.setBounds(key.x, key.y, key.x + key.width, key.y + key.height);
                    dr.draw(canvas);
                } else if (key.codes[0] == 73||key.codes[0] == 79) {  // notice 字母 I 和 O 设置特殊背景
                    Drawable dr = getContext().getResources().getDrawable(R.drawable.bg_btn_common_gray);
                    dr.setBounds(key.x, key.y, key.x + key.width, key.y + key.height);
                    dr.draw(canvas);
                    if (key.label != null) {
                        paint.setTextSize(ScreenUtils.sp2px(getContext(),18));
