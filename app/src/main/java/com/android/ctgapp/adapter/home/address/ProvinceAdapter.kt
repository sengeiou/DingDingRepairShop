 ...
 *描述: 城市选择器
 */
class ProvinceAdapter(private val context: Context, private val mList: List<CityEntity.ProvinceBean>,private var mProvince:String) : RecyclerView.Adapter<ProvinceAdapter.MyViewHolder>() {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.mSelectIV.visibility = if (data.name == mProvince) View.VISIBLE else View.GONE
        holder.mParentLL.setOnClickListener {
            mProvince = data.name.handlerNull()
            CitySelectManager.provinceSelectOver(data.name.handlerNull())
        }
        //处理首字母
        if (position != 0) {
