...
class GoodsMenuVinAdapter(private val context: Context, private val mList: List<GoodsByVinEntity.DataBean.FcodeVOListBean>, private val listener: MenuVinListener) : BaseRecyclerViewAdapter(context, mList) {
        ...
        if (holder is MyViewHolder) {
            ...
            holder.mTextLL.setOnClickListener {
                if (preSelect != position) {
                    listener.itemSelect(data.ctgnum.handlerNull())
    ...
    interface MenuVinListener {
        fun itemSelect(ctgnum: String)
    }
}