 ...
 * 描述: 横向group
 */
class BusinessFriendHorizontalGroupAdapter(private val context: Context, private val mList: ArrayList<BusinessFriendHorizontalItemEntity>, private val callback: SelectCallback) : BaseRecyclerViewAdapter(context, mList) {
    ...
    override fun handlerData(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder is MyViewHolder) {
            ...
            holder.mGroupNameTV.text = data.name.handlerNull()
            if (data.isSelect) {
            ...
            holder.mGroupLL.setOnClickListener {
                ...
                if (data.industry.isNotEmpty()) {
                    if (data.industry == "ALL") {
                        callback.findAll(data.industry, data.name)
                    } else {
                        callback.findByFixIndustry(data.industry, data.name)
                    }

                } else if (data.id != -1) {
                    callback.findByCustomId(data.id)
                }
    ...
    interface SelectCallback {
        fun findByFixIndustry(industry: String, industryName: String)
        fun findByCustomId(id: Int)
        fun findAll(industry: String, industryName: String)
    }
}